...
class PaperTradeEngine:
    ...
    def _start_db_writer(self) -> None:
        if self._db_write_queue is not None:
            return
        self._db_write_queue = Queue(maxsize=10000)
        self._db_write_thread = threading.Thread(
            target=self._db_write_loop,
            name="paper-trade-db-writer",
            daemon=True,
        )
        self._db_write_thread.start()

    def _db_write_loop(self) -> None:
        while not self._db_write_stop.is_set():
            try:
                kind, payload = self._db_write_queue.get(timeout=0.5) if self._db_write_queue is not None else ("", None)
            except Empty:
                continue
            try:
                self._run_db_write(kind, payload)
            except Exception as exc:
                self._db_write_errors += 1
                self._db_write_last_error = str(exc)[:255]
            finally:
                if self._db_write_queue is not None:
                    self._db_write_queue.task_done()

    def _enqueue_db_write(self, kind: str, payload: Any) -> None:
        if self._storage_backend != "mysql":
            return
        safe_payload = copy.deepcopy(payload)
        queue = self._db_write_queue
        if queue is None:
            self._run_db_write(kind, safe_payload)
            return
        try:
            queue.put_nowait((str(kind), safe_payload))
        except Full:
            self._run_db_write(kind, safe_payload)

    def _run_db_write(self, kind: str, payload: Any) -> None:
        if kind == "state":
            self._persist_mysql_snapshot(payload if isinstance(payload, dict) else {})
        elif kind == "feedback":
    ...
    def _persist(self) -> None:
        if self._storage_backend == "mysql":
            ...
            snapshot = {
                ...
                "last_candle_ts": int(self._last_candle_ts),
                "last_price": float(self._last_price),
                "delete_existing_trades": delete_existing,
            }
            self._persisted_closed_count = len(self._trades)
            self._enqueue_db_write("state", snapshot)

    def _persist_mysql(self) -> None:
        self._persist_mysql_snapshot(
            {
                "trades": copy.deepcopy(self._trades),
                "active_trade": copy.deepcopy(self._active_trade),
                "last_candle_ts": int(self._last_candle_ts),
                "last_price": float(self._last_price),
                "delete_existing_trades": len(self._trades) < self._persisted_closed_count,
            }
        )

    def _persist_mysql_snapshot(self, snapshot: Dict[str, Any]) -> None:
        trades = snapshot.get("trades") if isinstance(snapshot.get("trades"), list) else []
        active_trade = snapshot.get("active_trade") if isinstance(snapshot.get("active_trade"), dict) else None
        last_candle_ts = _to_int(snapshot.get("last_candle_ts"), 0)
        last_price = _to_float(snapshot.get("last_price"), 0.0)
        delete_existing = bool(snapshot.get("delete_existing_trades"))
        conn = None
        cur = None
        try:
            conn = self._connect_mysql(with_database=True)
            ...
            cur.execute(
                ...
                (
                    ...
                    int(last_candle_ts),
                    float(last_price),
                    json.dumps(active_trade) if active_trade is not None else None,
                    int(time.time()),
                ),
            )
            conn.commit()
    ...
    def _expected_sign(next_group: str, direction: str) -> int:
        ...
        # Support real pain groups plus compressed pain-side aliases.
        if group in {"put_buyers", "call_sellers", "bearish_pain", "bearish"}:
            return 1
        if group in {"call_buyers", "put_sellers", "bullish_pain", "bullish"}:
            return -1
    ...
    def _persist_feedback_row(self, row: Dict[str, Any]) -> None:
        self._enqueue_db_write("feedback", row)

    def _persist_feedback_row_mysql(self, row: Dict[str, Any]) -> None:
        if self._storage_backend != "mysql":
            return
        conn = None
        cur = None
    ...
    def _persist_mistake_row(self, row: Dict[str, Any]) -> None:
        self._enqueue_db_write("mistake", row)

    def _persist_mistake_row_mysql(self, row: Dict[str, Any]) -> None:
        if self._storage_backend != "mysql":
            return
        conn = None
        cur = None
    ...
    def _signal_to_direction(self, next_group: str) -> str:
        ...
        # Support real pain groups plus compressed pain-side aliases.
        if text in {"put_buyers", "call_sellers", "bearish_pain", "bearish"}:
            return "LONG"
        if text in {"call_buyers", "put_sellers", "bullish_pain", "bullish"}:
            return "SHORT"
        return ""
    ...
    def _state_locked(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "db_write_queue_depth": self._db_write_queue.qsize() if self._db_write_queue is not None else 0,
            "db_write_errors": int(self._db_write_errors),
            "db_write_last_error": str(self._db_write_last_error or ""),
